...
rule valik_split_ref:
	...
	shell:
		...
		( /usr/bin/time -a -o {params.log} -f "%e\t%M\t%x\tvalik-split\t{wildcards.b}\t{wildcards.fpr}\t{wildcards.er}\t{wildcards.min_len}" \
			{valik} split {input} --verbose --fpr {wildcards.fpr} \
				--out {output.ref_meta} --error-rate {wildcards.er}  \
...
f.write("time\tmem\terror-code\tcommand\tbins\tfpr\tmax-er\tmin-len\tthreads\tminimiser\tcmin\tcmax\terror-rate\trepeat-flag\tcart-max-cap\tmax-carts\trepeat-period\trepeat-length\trepeats\tmatches\n")
f.close()
...
rule valik_search:
	...
	shell:
		...
		(/usr/bin/time -a -o {params.log} -f \
			"%e\t%M\t%x\tvalik-search\t{wildcards.b}\t{wildcards.fpr}\t{wildcards.er}\t{wildcards.min_len}\t{threads}\t{params.is_minimiser}\t{wildcards.cmin}\t{wildcards.cmax}\t{wildcards.er}\t{repeat_flag}\t{wildcards.max_cap}\t{wildcards.max_carts}\t{wildcards.rp}\t{wildcards.rl}" \
		...
		{{ grep "Insufficient" {output}.search.err | test $1 = 1; }} | wc -l | awk '{{ print "\t" $1}}' >> {params.log}

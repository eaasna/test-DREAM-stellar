...
rule valik_split_ref:
	input:
		config["ref"]
	output: 
		ref_meta = "meta/k{k}_b{b}_fpr{fpr}_l{min_len}_e{er}.bin"
	params:
		log = "split_valik.time"
	shell:
		"""
		( /usr/bin/time -a -o {params.log} -f "%e\t%M\t%x\tvalik-split\t{wildcards.b}\t{wildcards.fpr}\t{wildcards.er}\t{wildcards.min_len}\t{wildcards.k}" \
...
rule valik_build:
	input:
		...
		ref_meta = "meta/k{k}_b{b}_fpr{fpr}_l{min_len}_e{er}.bin"
	output: 
		temp("/dev/shm/" + prefix + "/k{k}_b{b}_fpr{fpr}_l{min_len}_e{er}_cmin{cmin}_cmax{cmax}.index")
	params: 
	...
	shell:
		"""
		( /usr/bin/time -a -o {params.log} -f "%e\t%M\t%x\tvalik-build\t{wildcards.b}\t{wildcards.fpr}\t{wildcards.er}\t{wildcards.min_len}\t{wildcards.k}\t{workflow.cores}\t{params.is_minimiser}\t{wildcards.cmin}\t{wildcards.cmax}" \
...
rule valik_search:
	input:
		ibf = expand("/dev/shm/" + prefix + "/k{k}_b{b}_fpr{fpr}_l{min_len}_e{er}_cmin{cmin}_cmax{cmax}.index",
		query = config["query"],
		ref_meta = "meta/k{k}_b{b}_fpr{fpr}_l{min_len}_e{er}.bin"
	output:
	...
	shell:
		...
		(/usr/bin/time -a -o {params.log} -f \
			"%e\t%M\t%x\tvalik-search\t{wildcards.b}\t{wildcards.fpr}\t{wildcards.er}\t{wildcards.min_len}\t{wildcards.k}\t{threads}\t{params.is_minimiser}\t{wildcards.cmin}\t{wildcards.cmax}\t{wildcards.er}\t{repeat_flag}\t{wildcards.max_cap}\t{wildcards.max_carts}\t{wildcards.rp}\t{wildcards.rl}" \

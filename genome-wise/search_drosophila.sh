...
#(timeout $timeout /usr/bin/time -a -o $log -f "%e\t%M\t%x\tvalik-search\tb=${ibf_bins}\tk=${kmer_size}\tq=${query_seg_count}\tl=${min_len}\ter=${er}\tt=${threshold}"  
valgrind $valik search --without-parameter-tuning --seg-count $query_seg_count --threshold $threshold --pattern $min_len --split-query --cache-thresholds --numMatches $numMatches --sortThresh $sortThresh --index $index --ref-meta $ref_meta --query $query --error-rate $er --threads $threads --output $out --cart-max-capacity 10000 --max-queued-carts 4096 > valgrind.out 2>&1 

#|| touch $out ) > ${timeout}_${prefix}.log

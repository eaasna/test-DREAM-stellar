...
do
	echo "#keep repeats from bins with highest entropy" >> $log_file
done
...
echo -e "time\tmem\terror-code\tcommand\tbins\tthreads\ttau\tp_max\tcart-max-cap\tmax-queued\tmin-len\ter\tcmin\tcmax\trepeat-mask\tbin-entropy-cutoff\tibf-fpr\tibf-size\trepeats\tmatches\ttruth-set-matches\ttrue-matches\tmissed\tmin_overlap" >> $search_log

ibf_fpr=0.005
	
#################### SPLIT ####################
echo "Splitting reference database"
/usr/bin/time -a -o $split_log -f "%e\t%M\t%x\tvalik-split\t${max_er}\t${min_len}\t${ibf_bins}\t$ibf_fpr" $valik split $ref --verbose --fpr $ibf_fpr --out $ref_meta --error-rate $max_er --pattern $min_len -n $ibf_bins &> "${prefix}_b${ibf_bins}_fpr${ibf_fpr}_split.err"
...
bin_entropy_cutoff=0.1
for tau in 0.6 0.999
do
for p_max in 0.05 0.15
do
for er in 0.053
do
	#truth_file="mouse_vs_fly_stellar_l${min_len}_e${er}_filter_repeats.gff" 
	truth_file="../blast/blast_e1_k16.txt"

if [ -f $truth_file ]; then
for repeat_mask in "--keep-best-repeats"
do
	#(timeout $timeout /usr/bin/time -a -o $stellar_log -f "%e\t%M\t%x\tstellar\t${min_len}\t${er}" $stellar --repeatPeriod 2 --repeatLength 50 -a dna --numMatches $numMatches --sortThresh $sortThresh $ref $query -e $er -l $min_len -o $truth_file)

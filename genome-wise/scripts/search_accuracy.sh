...
if [[ "$TRUTHFILE_TYPE" == "gff" ]]; then
	awk '{print $4 "\t" $5}' $TRUTH > $TRUTH_TMP
	#awk '{ print $1 "\t" $4 "\t" $5 "\t" $9}' $TRUTH | awk -F";" '{ print $1 "\t" $2}' | awk -F"seq2Range=" '{ print $1 "\t" $2}' | awk -F"," '{ print $1 "\t" $2}' > $TRUTH_TMP
else
	awk '{print $2 "\t" $3}' $TRUTH > $TRUTH_TMP
fi
...
total_match_count=$(wc -l "$TRUTH_TMP" | awk '{ print $1 }')
grep -v -f $MATCHES.begin.tsv $TRUTH_TMP | grep -v -f $MATCHES.end.tsv > $MATCHES.still.searching
rm $TRUTH_TMP

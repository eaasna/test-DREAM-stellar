...
echo -e "time\tmem\terror-code\tcommand\tbins\tk\tquery-seg\tcart-max-cap\tmin-len\ter\tthresh\tcmin\tcmax\tbin-entropy-cutoff\tibf-size\tmatches\trepeats\tturth-set-matches\ttrue-matches\tmissed" >> $search_log
...
do
	...
	(timeout $timeout /usr/bin/time -a -o $search_log -f "%e\t%M\t%x\tvalik-search\t${ibf_bins}\t${kmer_size}\t${query_seg_count}\t${cart_max_cap}\t${min_len}\t${er}\t${threshold}\t${kmer_cmax}\t${bin_entropy_cutoff}" $valik search $repeat_mask --bin-entropy-cutoff $bin_entropy_cutoff --without-parameter-tuning --verbose --seg-count $query_seg_count --threshold $threshold --pattern $min_len --split-query --cache-thresholds --numMatches $numMatches --sortThresh $sortThresh --index $index --ref-meta $ref_meta --query $query --error-rate $er --threads $threads --output $out --cart-max-capacity $cart_max_cap --max-queued-carts $ibf_bins || touch $out ) > ${timeout}_${prefix}.log 2> ${prefix}.err

...
prefix="test-kmer-seg"
mkdir -p /dev/shm/$prefix

log="${prefix}_valik_search.time"
...
truth_file="mouse_vs_fly_l${min_len}_e${er}.gff"

function run_manual_split() {
	kmer_size=$1
	ibf_bins=$2
	run_id="${prefix}_k${kmer_size}_b${ibf_bins}"
...
for bins in 64 512 1024 2048
do
for k in 19 21
do	
	ref_meta=$(run_manual_split $k $bins)
	for cmin in 0 1
	do
